...
/*   Updated: 2022/08/20 16:43:21 by skorte           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::signForm(Form& form) {
	try {
		...
		form.ErrorGradeTooLowToSign(*this);
		return ;
	}
	catch (Form::AlreadySignedException& exc){
		std::cout << "# " << exc.what() << std::endl;
		form.ErrorAlreadySigned(*this);
		return ;
	}
	form.SuccessSign(*this);
	return ;
}

void	Bureaucrat::executeForm(Form const & form){
	if (_grade <= form.getGradeToExecute() && form.getSigned())
		form.SuccessExecute(*this);
	try {
		form.execute(*this);
	}
	catch (Form::GradeTooLowException& exc){
		std::cout << "# " << exc.what() << std::endl;
		form.ErrorGradeTooLowToExecute(*this);
		return ;
	}
	catch (Form::NotSignedException& exc){
		std::cout << "# " << exc.what() << std::endl;
		form.ErrorNotSignedYet(*this);
		return ;
	}
	return ;
}
